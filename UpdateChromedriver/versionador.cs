...
namespace UpdateChromeDriver
{
    public class VersaoChromeDriver
    {
        ...
        private bool consultaarquivo(string arquivo)
        {
            ...
            else
            {
                using (var client = new WebClient())
                {
                    ...
                    string LATESTFolder = $@"{caminho}\LATEST\";
                    System.IO.DirectoryInfo diLATEST = new DirectoryInfo(LATESTFolder);
                    ...
                    {
                        foreach (FileInfo fileItem in diLATEST.GetFiles())
                        {
                            file.Delete();
                        }
                    ...
                    File.Copy($@"{caminho}\{versao}\chromedriver.exe", $@"{LATESTFolder}\chromedriver.exe");
